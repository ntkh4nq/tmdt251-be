...
class Order(Base):
    ...
    status: Mapped[Order_Status] = mapped_column(SQLEnum(Order_Status), default=Order_Status.PENDING)
    payment_status: Mapped[Payment_Status] = mapped_column(SQLEnum(Order_Payment_Status), default=Order_Payment_Status.UNPAID)
    ...
    shipments: Mapped[List["Shipment"]] = relationship(back_populates="order", lazy="selectin")
...
class Payment(Base):
    ...
    status: Mapped[Payment_Status] = mapped_column(SQLEnum(Payment_Status), default=Payment_Status.PENDING)
    method: Mapped[Payment_Method] = mapped_column(SQLEnum(Payment_Method), default=Payment_Method.BANK)
...
class Shipment(Base):
    ...
    shipped_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    status: Mapped[Shipment_Status] = mapped_column(SQLEnum(Shipment_Status), default=Shipment_Status.PREPARING)
...
class OrderItem(Base):
    ...
    __table_args__ = (UniqueConstraint("product_id", "order_id", name="uq_product_order"))
