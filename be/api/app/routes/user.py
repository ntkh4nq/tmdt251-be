...
async def login_user(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User email not found, please register")
    ...
    refresh_token: str = create_refresh_token(data={"sub": user.email}, settings=settings)
    device_info: str = request.headers.get("User-Agent", "unknown device")
    expried_at: datetime = datetime.now(timezone.utc)+timedelta(days=settings["REFRESH_TOKEN_EXPIRE_DAYS"])
    new_session = Session(
...
async def refresh_token(
    ...
    session = await db.execute(select(Session).where(Session.refresh_token == refresh_token))
...
async def logout(
    ...
    session_query = await db.execute(
        select(Session)
        .where(and_(Session.refresh_token == refresh_token, Session.user_id == current_user.id))
    )
